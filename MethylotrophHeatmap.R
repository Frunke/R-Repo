...
MethylPotential <- read.csv("TR_Metabolism_Summary_METHYL_Trim.csv")
...
#Methyl-N
a <- ggplot(MethylN, aes(x = variable, y=MAGs , fill=value))+
  geom_tile(color='black', lwd=.15, linetype=1)+
  theme_minimal()+
  theme(
    plot.title = element_text(hjust = 0.5, size = 16),
  ...
  scale_fill_continuous(low="white", high="darkblue", na.value="white", limits = c(0,max(MethylN$value)))+
  xlab(NULL) + labs(title = NULL, fill = "# of Genes")+
  scale_y_discrete(limits=rev)+
  theme(legend.position = "bottom")
#Methyl-O
b <- ggplot(MethylO, aes(x = variable, y=MAGs , fill=value))+
  geom_tile(color='black', lwd=.15, linetype=1)+
  theme_minimal()+
  theme(
    plot.title = element_text(hjust = 0.5, size = 16),
  ...
  scale_fill_continuous(low="white", high="red", na.value="white", limits = c(0,max(MethylO$value)))+
  xlab(NULL) + labs(title = NULL, fill = "# of Genes")+
  scale_y_discrete(limits=rev)+
  theme(legend.position = "bottom")
#Methyl-S
c <- ggplot(MethylS, aes(x = variable, y=MAGs , fill=value))+
  geom_tile(color='black', lwd=.15, linetype=1)+
  theme_minimal()+
  theme(
    plot.title = element_text(hjust = 0.5, size = 16),
  ...
  scale_fill_continuous(low="white", high="gold", na.value="white", limits = c(0,max(MethylS$value)), breaks = c(0,2,4,6,8,10))+
  xlab(NULL) + labs(title = NULL, fill = "# of Genes")+
  scale_y_discrete(limits=rev)+
  theme(legend.position = "bottom")
  

MethylPlot <- grid.arrange(a, b, c, ncol = 3, layout_matrix = rbind(c(1, 2,3)))
